...
public:
	...
	bool WriteTileMetadata();
	void CreatMBTiles(const std::string& filename);
	bool WriteTile(unsigned int zoomLevel, unsigned int tileColumn, unsigned int tileRow, const std::vector<uint8_t>& blobOut);
...
//Singleton& globalDBSingleton = Singleton::getInstance();
...
public:
	...
	//返回要写进mbtiles的二进制
	std::vector<uint8_t> writeMBTIlesFile() const;
